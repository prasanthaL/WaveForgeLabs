  ...
  return (
    <section id="about" className="relative w-full py-24 bg-slate-950 px-6 lg:px-12 overflow-hidden border-t border-white/5">
      ...
      <div className="max-w-5xl mx-auto relative z-10">
        ...
        <motion.div 
          className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-12 items-center mb-32 pb-16 border-b border-white/5"
          initial="hidden"
          whileInView="visible"
            ...
            {FORGE_STEPS.map((step, index) => {
              ...
              return (
                <div key={step.number} className={`flex flex-col md:flex-row items-center relative ${isEven ? "md:flex-row-reverse" : ""}`}>
                  ...
                  {/* Card Block with slide transition */}
                  <motion.div 
                    className="w-full md:w-1/2 px-0 md:px-12"
                  ...
                  >
                    <span className="text-6xl sm:text-8xl font-black font-mono tracking-tighter text-white select-none hover:text-white/20 transition-colors duration-300">
                      {step.number}
                    </span>
                  </motion.div>
